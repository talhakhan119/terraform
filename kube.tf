...
module "kube-hetzner" {
  ...
  agent_nodepools = [
    ...
    # },
    # {
    #   name                 = var.agent_node_name
    #   server_type          = var.agent_node_server_type
    #   location             = var.agent_node_location
    #   labels               = []
    #   taints               = []
    #   count                = var.agent_node_count
    #   longhorn_volume_size = var.agent_node_longhorn_volume_size
  ...
  ingress_target_namespace = "nginx-controller"
...
terraform {
  ...
  required_providers {
    ...
    # backend "s3" {
    # bucket = "hetzner-k3s-perceptdata-cfg"
    # key    = "terraform/terraform-perceptdata-k3s.tfstate"
    # region = "eu-west-1"
  # }
}
